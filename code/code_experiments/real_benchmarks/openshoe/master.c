...
#define ALLOCATION_ALG 1 //PERFORMANCE = 1, ENERGY = 2
...
static const float P_lc_active_freq[3][6] = {
	{12.6889,	14.0343,	15.911,		17.1577,	19.5965,	19.7577},
	{14.5264,	16.2673,	19.0652,	21.3612,	22.8991,	25.7348},
	{12.6434,	14.2359,	16.8462,	19.3088,	21.7133,	22.6478}
};
...
int main(void)
{
	...
	Deltas[0] = 1;
	average_time = (M0_time_ms[1] + M0_time_ms[2])*0.5;
	Deltas[1] =  (M0_time_ms[2] - average_time) * 1000 / average_time * 0.001;
	...
	for(f_lc = F_LC_MIN; f_lc <= F_LC_MAX; f_lc+=F_LC_STEP)
	{
		...
		for(i=0; i<ITERATIONS+1; i++)
		{
			...
			{
				zupt_detector();
				long slave_t =  M0_time_ms[1] * TIME_1s_100MHZ * expansion_factor*0.001;
#ifndef PERFORMANCE
				P_lc_active = P_lc_active_freq[1][(int) (f_lc - 50) / 10] +
						(P_lc_active_freq[1][(int) (f_lc - 50) / 10 + 1] - P_lc_active_freq[1][(int) (f_lc - 50) / 10]) *
						10000 / 10 * ((int) f_lc % 10) * 0.0001;
				slave_t = smart_expansion_factor(slave_t, M0_time_ms[2]);
#endif
				mutexTake();
			...
			else if(ALLOCATION_ALG == 2)
			{
				//zupt_detector
				P_hc_active = 28.251;
				P_lc_active = P_lc_active_freq[0][(int) (f_lc - 50) / 10] +
										(P_lc_active_freq[0][(int) (f_lc - 50) / 10 + 1] - P_lc_active_freq[0][(int) (f_lc - 50) / 10]) *
										10000 / 10 * ((int) f_lc % 10) * 0.0001;
				Delta = Deltas[0];
				...
				{
					long slave_t =  M0_time_ms[0] * TIME_1s_100MHZ * expansion_factor*0.001;
#ifndef PERFORMANCE
					slave_t = smart_expansion_factor(slave_t, 0);
#endif
					mutexTake();
					Chip_MBOX_SetValue(LPC_MBOX, otherCoreBox, slave_t);
					mutexGive();
				...
				//gain_matrix and measurement_update


				P_hc_active = 34.559;
				float temp1 = P_lc_active_freq[1][(int) (f_lc - 50) / 10] +
						(P_lc_active_freq[1][(int) (f_lc - 50) / 10 + 1] - P_lc_active_freq[1][(int) (f_lc - 50) / 10]) *
						10000 / 10 * ((int) f_lc % 10) * 0.0001;
				float temp2 = P_lc_active_freq[2][(int) (f_lc - 50) / 10] +
						(P_lc_active_freq[2][(int) (f_lc - 50) / 10 + 1] - P_lc_active_freq[2][(int) (f_lc - 50) / 10]) *
						10000 / 10 * ((int) f_lc % 10) * 0.0001;
				P_lc_active = (temp1 + temp2)*0.5;
				Delta = Deltas[1];
				...
				{
					long slave_t =  M0_time_ms[2] * TIME_1s_100MHZ * expansion_factor*0.001;
#ifndef PERFORMANCE
