...
/*Do not forget to modify the Deltas table below according to your experiment*/
#define PDOWNMODE   (POWER_SLEEP)
#define TIME_1s_100MHZ 100000000
#define PERIOD_ms 100
#define UTIL_MAX_PER_CENT 50
#define UTIL_MIN_PER_CENT 50
#define UTIL_STEP_PER_CENT 10
#define F_LC_MAX 100
#define F_LC_MIN 80
#define F_LC_STEP 20
#define ITERATIONS 10
#define ALLOCATION_ALG 3 // MAX_PAR = 1, PERFORMANCE = 2, ENERGY = 3
...
float Deltas[21]; 	//0.05 step for the Makespan/Energy as a function of delta simulation

/* for the Makespan/Energy as a function of utilization simulation
  Do not forget to remove the previous declaration and the initialization of the Delta table in the body
  if you want to conduct a utilization experiment */
/*float Deltas = {	
	0.1,
	0.5
};*/
...
void max_parallelism_allocation(void)
{
	float f_scale_master = f_hc*100000/(f_hc + f_lc)*0.00001;
	float f_scale_slave = 1 - f_scale_master;
	float expansion_factor = f_hc*100000/f_lc*0.00001;

	master_time = single_core_workload * f_scale_master;
	slave_time = single_core_workload * f_scale_slave * expansion_factor;
}

void performance_allocation_policy(void)
{
	float expansion_factor = f_hc*100000/f_lc*0.00001;
	master_time = single_core_workload * (1 + Delta) * 0.5;
	slave_time = single_core_workload * (1 - Delta) * expansion_factor * 0.5;
}
...
int main(void)
{
	...
	/* Remove in the case of the utilization experiments */
	Deltas[0] = 0;
	for(i=1;i<=20;i++)
	{
		Deltas[i] = Deltas[i-1] + 0.05;
	}
	...
	for(f_lc = F_LC_MIN; f_lc <= F_LC_MAX; f_lc+=F_LC_STEP)
	{
		...
		for(j=0; j<sizeof(Deltas)/sizeof(float); j++)
		{
			Delta = Deltas[j];
			...
			for(util_per_cent = UTIL_MIN_PER_CENT; util_per_cent <= UTIL_MAX_PER_CENT; util_per_cent+=UTIL_STEP_PER_CENT)
			{
				...
				{
					int slave_works = !(Delta >0.99 && slave_time < master_time);

					if(slave_works == 1)
					{
						mutexTake();
						Chip_MBOX_SetValue(LPC_MBOX, otherCoreBox,  slave_time * TIME_1s_100MHZ);
						mutexGive();
					}

					task(master_time*TIME_1s_100MHZ);
					go_to_sleep_M4 = 1;

#ifdef PERFORMANCE
	Chip_GPIO_SetPinState(LPC_GPIO, 0, 4, false);
#endif
					while(go_to_sleep_M0 == 0 && slave_works == 1) {
						__WFI();
					}
	...
	Chip_GPIO_SetPinState(LPC_GPIO, 0, 4, true);
					...
					Chip_SYSCON_PowerUp(SYSCON_STARTER_RIT);
					Chip_SYSCON_PowerUp(SYSCON_STARTER_MAILBOX);
