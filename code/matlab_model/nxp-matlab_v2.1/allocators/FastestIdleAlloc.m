classdef FastestIdleAlloc < Allocator
    %FASTESTIDLEALLOC This allocation policy always selects the
    %Fastest cpu among the available CPU, going to the slower cores only
    %when faster ones are busy.
    %   At any given time, a job is assigned the fastest available CPU
    ...
    methods
        ...
        function obj = FastestIdleAlloc()
            obj = obj@Allocator();
            obj.name = 'Fastest (P)';
            obj.long_name = 'Fastest Idle First';
        end
        ...
        %deadline
        function [alloc_m, cpu] = fastest_idle(obj, procs, alloc_m, time, job) 
           cpu = 1; 
           ids = (1:1:obj.n_procs).';
           ...
           %flip to have cpu's in descending freq order
           cpu_m = flipdim(sortrows(cpu_m, 1),1);
        ...
        function [job_queue, cpu_alloc] = allocate(obj, procs, job_queue, job_set_index, time)
            ...
            for i=1:length(job_set)
                [alloc_m, cpu_alloc(i)] = obj.fastest_idle(procs, alloc_m, time, job_set(i));
