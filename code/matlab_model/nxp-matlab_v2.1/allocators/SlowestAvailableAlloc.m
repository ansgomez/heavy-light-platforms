classdef SlowestAvailableAlloc < Allocator
    %SLOWESTAVAILABLEALLOC This allocation policy always selects the
    %slowest cpu (that meets the deadline) among the available CPU
    %   At any given time, a job is assigned the slowest CPU which will be
    ...
    methods
        ...
        function obj = SlowestAvailableAlloc()
            obj = obj@Allocator();
            obj.name = 'slowest\_available';
            obj.long_name = 'Slowest Available First';
        end
        ...
        %deadline
        function [alloc_m, cpu] = slowest_available(obj, procs, alloc_m, time, job) 
           cpu = 1; 
           ids = (1:1:obj.n_procs).';
        ...
        function [job_queue, cpu_alloc] = allocate(obj, procs, job_queue, job_set_index, time)
            ...
            for i=1:length(job_set)
                [alloc_m, cpu_alloc(i)] = obj.slowest_available(procs, alloc_m, time, job_set(i));
