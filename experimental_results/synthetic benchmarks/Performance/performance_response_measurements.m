function aux = performance_response_measurements(file, total_iterations, util_measurements, delta_measurements, frequency_measurements, sampling_rate, f_min)
	...
	for i=1:size(data,1)
		if data(i,1) > V_START && data(i,2) > V_LED_ON
			k = i;
			break
		end
	end
	...
	for i=k:size(data,1)
		if f_min == 50 && frequency == 1 && util == 10 && delta == 1
			...
			if active == 1 && data(i,2) < V_LED_ON && data(i,3) < V_LED_ON
				active = 0;
				stop = i;
			elseif active == 0 && data(i,2) > V_LED_ON 
