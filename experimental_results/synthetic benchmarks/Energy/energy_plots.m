...
FREQ_TABLE = [70 80 90 100]; %the frequencies for which we are going to plot the graph

%Specify the name of the input files
single_core_energy_file = 'sc_meas1.xlsx';
max_par_energy_file = 'max_par_meas1.xlsx';
performance_energy_file = 'performance_meas1.xlsx';
energy_energy_file = 'energy_meas1.xlsx';
...
for k=1:size(FREQ_TABLE)
	...
	for d=1:DELTA_MEASUREMENTS
		y(d) = (energy_singlecore(util) - energy_max_par(freq_index,d,util)) / energy_singlecore(util) * 100;
	end
	title_name = sprintf('Energy Saving - Delta, Utilization = %.1f', UTIL_PER_CENT / 100);
	...
	legend('-DynamicLegend', 'Location', 'Best');
	for d=1:DELTA_MEASUREMENTS
		y(d) = (energy_singlecore(util) - energy_performance(freq_index,d,util)) / energy_singlecore(util) * 100;
	end
	...
	legend('-DynamicLegend', 'Location', 'Best');
	for d=1:DELTA_MEASUREMENTS
		y(d) = (energy_singlecore(util) - energy_energy(freq_index,d,util)) / energy_singlecore(util) * 100;
	end
